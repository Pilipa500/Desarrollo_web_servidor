...
<body>
	...
	function validateName($name){
		...
		if (preg_match("/^[A-Za-z ]{3,15}$/", $name)) {
			...
			echo "El nombre no es válido, tiene que tener un mínimo de 3 caracteres y un máximo de 15";
			return false;
		}
	}
	...
	if (!empty($_POST['nombre'])) { //Aqui se comprueba que el nombre no esté vacío, si no está vacío se ejecuta el bucle for
		...
		if (validateName($_POST['nombre']) && validateNumero($_POST['telefono'])) {//Aquí se comprueba que el nombre y el número introducido sea válido
			...
			$agenda[$posr + 1] = $_POST['telefono']; //aqui se reemplaza el número de telefono por el indicado por el formulario
		}
		...
		if ($val == true && empty($_POST['telefono'])) {
			...
			array_splice($agenda, $posrepe, 2); //aqui con esta función cojo dentro del array $agenda la posición que tiene el valor repetido y le digo mediante el 2 que quiero que me seleccione esa posición y la siguiente, y como no le indico nada más las borra. 
